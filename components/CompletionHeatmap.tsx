...
export default function CompletionHeatmap({ 
  ...
  const option: EChartsOption = {
    ...
    visualMap: {
      ...
      inRange: {
        color: [
          '#334155', // slate-700 (low)
          '#1e40af', // blue-800 (medium)
          '#4f46e5', // indigo-600 (high)
          '#7c3aed'  // purple-600 (very high)
        ]
      }
    },
    series: [
    ...
    <div className="flex flex-col">
      <div className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-700/30 rounded-lg p-4 text-center">
          ...
          <div className="text-xl font-bold text-indigo-400">{formatDay(mostActiveDay.day)}</div>
          <div className="text-sm text-gray-400">{mostActiveDay.count} tasks</div>
        </div>
        
        <div className="bg-gray-700/30 rounded-lg p-4 text-center">
          <div className="text-xs text-gray-400 mb-1">Most Active Hour</div>
          <div className="text-xl font-bold text-indigo-400">{formatHour(mostActiveHour.hour)}</div>
          <div className="text-sm text-gray-400">{mostActiveHour.count} tasks</div>
        </div>
        ...
        <div className="bg-gray-700/30 rounded-lg p-4 text-center">
          ...
          <div className="text-sm text-gray-400">{mostActiveCombination.count} tasks</div>
        </div>
      </div>
